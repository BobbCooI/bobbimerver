  ...
  async choose(choice: string, id: string): Promise<VRVret> {
    ...
    this.cache[id].aniEps = episodes.res.body.items.map(
      (ep: mediaResourceJson, index: number): string =>
        `${ordinate(index + 1)} Episode　•　${ep.title}　•　${ep.__links__.streams.href.split("/")[
        ep.__links__.streams.href.split("/").length - 2
        ]}`
    );
    return { success: true, res: this.cache[id].aniEps, title: this.cache[id].choiceTitle };
  }
  async getStream(eps: Array<number>, message: Message, id: string): Promise<VRVret> {
    let st = Date.now();
    ...
    for (let epNum of eps) {
      ...
      if (!streams.success)
        return {
          success: false,
          error: `Error while getting the streams for ${this.cache[id].selEp?.split("　•　")[1]
            }`
        };
      ...
      } else return { success: false, error: `Trouble parsing for streams for episode ${epNum}` };
     }
  ...
  async _vGetData(options: getOptions): Promise<VRVret> {
    ...
    try {
      ...
      await this.Bobb.loggers.log(
        `[INFO] VRV requesting to ${gOptions.prefixUrl
          ? gOptions.prefixUrl + "/" + gOptions.url
          : gOptions.url
