...
export default class ArgManager {
  ...
  args: Array<string>;
  bot: Client;
  constructor(msg: Message, args: Array<string>) {
  ...
  resolveUser(
    ...
    if (idMatch) {
      ...
      ret = (this.msg.channel as GuildChannel).guild.members.cache.get(
      ...
      if (args.length > 5 && args.slice(-5, -4) === "#") {
        // we have a discrim
        ret = (this.msg.channel as GuildChannel).guild.members.cache.find(
          (member:GuildMember) =>
  ...
  resolveChannel(
    ...
    if (idMatch) {
      ...
      if (!(this.msg.channel as GuildChannel)?.guild) {
  ...
  resolveRole(consumeRest: boolean = false): Role |null {
    ...
    if (!(this.msg.channel as GuildChannel).guild || !args) {
      return null;
    }
    ...
    if (idMatch) {
      return (this.msg.channel as GuildChannel).guild.roles.cache.get(idMatch[1] as Snowflake) || null;
    } else {
      return (this.msg.channel as GuildChannel).guild.roles.cache.find(
  ...
  cleanContent(consumeRest: boolean = false): null | undefined | string {
    ...
    while ((match = userMentionMatcher.exec(args)) !== null) {
      // Clean user mentions
      const user: GuildMember | User | undefined = this.msg.guild
        ? this.msg?.guild.members.cache.get(match[1] as Snowflake)
    ...
    while ((match = channelMentionMatcher.exec(args)) !== null) {
      ...
      const channel =( this.msg.channel as GuildChannel).guild.channels.cache.get(match[1] as Snowflake);
    ...
    while ((match = roleMentionMatcher.exec(args)) !== null) {
      // Clean role mentions
      const role = (this.msg.channel as GuildChannel).guild.roles.cache.get(match[1] as Snowflake);
