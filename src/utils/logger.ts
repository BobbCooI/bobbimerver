...
import GenericCommand from '../bot/commandTypes/GenericCommand';
...
await chan.send({content: msg}).catch((e:any):any=> console.log("message:", message, "\nerror:", e.message));
  ...
  async reportError(e: any, message: Message, command: GenericCommand):Promise<void> {
  let date = new Date();
  ...
  await Bobb.botStats.updateOne(
    ...
    let msg = await Bobb.misc.errorMessages(e.toString());
